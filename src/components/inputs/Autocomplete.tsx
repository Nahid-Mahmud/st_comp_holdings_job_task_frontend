...
}>(({ theme, customWidth, customMinHeight }) => ({
  width: customWidth || '300px',
  minHeight: customMinHeight || 'auto',
...
export function CustomAutocomplete<Value>(
  ...
  const { width, minHeight, renderOption, ...autocompleteProps } = props;
  const {
    getRootProps,
    getInputLabelProps,
    getInputProps,
          ...
          {value.map((option, index) => {
            ...
            return (
              <StyledItem
                key={key}
                {...itemProps}
                label={autocompleteProps.getOptionLabel!(option)}
              />
            );
          })}
          <input {...getInputProps()} />
          ...
          {groupedOptions.map((option, index) => {
            ...
            return (
              <li key={key} {...optionProps}>
                {renderOption ? (
                  <>
                    <div className="option-content">{renderOption(option)}</div>
                    <CheckIcon fontSize="small" className="check-icon" />
                  </>
                ) : (
                  <>
                    <span>{autocompleteProps.getOptionLabel!(option)}</span>
                    <CheckIcon fontSize="small" className="check-icon" />
                  </>
                )}
              </li>
